...
int main() {
	...
	/*std::vector<Option*> opt_ptrs2;
	opt_ptrs2.push_back(new CallOption(_T, _K));
	opt_ptrs2.push_back(new PutOption(_T, _K));
	opt_ptrs2.push_back(new DigitalCallOption(_T, _K));
	opt_ptrs2.push_back(new DigitalPutOption(_T, _K));
	opt_ptrs2.push_back(new AmericanCallOption(_T, _K));
	opt_ptrs2.push_back(new AmericanPutOption(_T, _K));

	CRRPricer* pricer6;

	for (auto& opt_ptr : opt_ptrs2)
	{
		pricer6 = new CRRPricer(opt_ptr, 10, _S0, _r, _sigma);

		pricer6->compute();
		
		std::cout << "price: " << (*pricer6)(false) << std::endl << std::endl;
		delete pricer6;
		delete opt_ptr;

	}

	std::cout << "The prices obtained at this step are abnormal because of the synthax used there." << std::endl;
	std::cout <<  "Nevertheless, the CRR still works when computing options price in the 'classical' way "<< std::endl << std::endl;*/
